...
-   OBS服务会因欠费停服，从而导致直播录制失败，建议您[购买OBS存储包](https://storage.huaweicloud.com/obs/?#/obs/buy?status=1)。
...
6.  请您按照实际需求配置录制参数，参数说明如[表1](#table1985784519104)所示。
    ...
    **表 1**  录制参数说明
    ...
    <a name="ul0962258554"></a><a name="ul0962258554"></a><ul id="ul0962258554"><li>断流后生成新文件：是指录制的直播流中断后，会立即生成新的录制文件。</li><li>断流后不生成新文件：是指录制的直播流中断后，会和之前录制的文件合并为一个文件。最大断流合并时长为300秒。</li><li>其他数值：是指录制的直播流中断时间在设置范围内，则和之前录制的文件合并为一个文件，否则，生成新的录制文件。</li></ul>
    </td>
    </tr>
