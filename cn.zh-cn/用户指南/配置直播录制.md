...
-   已添加经过备案的域名，具体请参见[配置直播域名](配置直播域名.md)。且已完成CNAME解析配置，具体请参见[配置CNAME](配置CNAME.md)。
...
-   直播录制生成的视频文件存储在OBS中，因此，产生的相关费用是按OBS的价格进行计费收取的，具体价格详情请参见[OBS-产品价格详情](https://www.huaweicloud.com/pricing.html?tab=detail#/obs)。
-   若OBS桶授权给点播服务后，在点播服务则对托管的录制文件进行了转码、截图、转封装或音频提取等媒资处理产生的相关费用是按点播的价格进行计费收取的，具体价格详情请参见[视频点播-产品价格详情](https://www.huaweicloud.com/pricing.html?tab=detail#/vod)。
...
OBS桶授权直播服务，允许直播录制文件存储在OBS桶中，若需要对存储的录制文件进行转码、截图、转封装或音频提取等媒资处理时，选择授权点播服务，将存储在OBS桶中的录制文件托管给点播服务。
...
4.  在左侧导航树中选择“模板配置 \> 录制配置”，进入录制配置页面。
    ...
    若您需要实时了解直播录制的状态，可以单击“添加”设置回调地址，如[图2](#fig2051893015542)所示。录制文件回调消息体请参见[录制回调](录制回调.md)。
